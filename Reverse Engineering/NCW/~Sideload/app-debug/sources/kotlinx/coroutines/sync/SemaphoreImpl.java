...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        ...
        while (true) {
            ...
            while (true) {
                if (cur$iv$iv.getId() < id$iv || cur$iv$iv.getRemoved()) {
                    ...
                    m1718constructorimpl = SegmentOrClosed.m1718constructorimpl(cur$iv$iv);
                    curTail = s$iv2;
                    break;
                }
            }
            s$iv = m1718constructorimpl;
            if (SegmentOrClosed.m1723isClosedimpl(s$iv)) {
                break;
            }
            Segment to$iv$iv = SegmentOrClosed.m1721getSegmentimpl(s$iv);
            while (true) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m1721getSegmentimpl(s$iv);
