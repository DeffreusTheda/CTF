...
public final class TextFieldSelectionManagerKt {
    ...
    public static final boolean isSelectionHandleInVisibleBound(TextFieldSelectionManager textFieldSelectionManager, boolean z) {
        ...
        return SelectionManagerKt.m975containsInclusiveUv8p0NA(visibleBounds, textFieldSelectionManager.m994getHandlePositiontuRUvjQ$foundation_release(z));
    }

    /* renamed from: calculateSelectionMagnifierCenterAndroid-O0kMr_c, reason: not valid java name */
    public static final long m995calculateSelectionMagnifierCenterAndroidO0kMr_c(TextFieldSelectionManager manager, long j) {
        int m4532getStartimpl;
        ...
        if (i == 1 || i == 2) {
            m4532getStartimpl = TextRange.m4532getStartimpl(manager.getValue$foundation_release().getSelection());
        } else {
            if (i != 3) {
                throw new NoWhenBranchMatchedException();
            }
            m4532getStartimpl = TextRange.m4527getEndimpl(manager.getValue$foundation_release().getSelection());
        }
        int originalToTransformed = manager.getOffsetMapping().originalToTransformed(m4532getStartimpl);
        ...
        long m2476getCenterF1C5BW0 = value.getBoundingBox(coerceIn).m2476getCenterF1C5BW0();
        ...
        Offset m992getCurrentDragPosition_m7T9E = manager.m992getCurrentDragPosition_m7T9E();
        if (m992getCurrentDragPosition_m7T9E == null) {
            return Offset.INSTANCE.m2461getUnspecifiedF1C5BW0();
        }
        float m2446getXimpl = Offset.m2446getXimpl(innerTextFieldCoordinates.mo3975localPositionOfR5De75A(layoutCoordinates, m992getCurrentDragPosition_m7T9E.getPackedValue()));
        ...
        boolean z = TextRange.m4532getStartimpl(manager.getValue$foundation_release().getSelection()) > TextRange.m4527getEndimpl(manager.getValue$foundation_release().getSelection());
        ...
        float coerceIn2 = RangesKt.coerceIn(m2446getXimpl, Math.min(horizontalPosition, horizontalPosition2), Math.max(horizontalPosition, horizontalPosition2));
        if (Math.abs(m2446getXimpl - coerceIn2) > IntSize.m5177getWidthimpl(j) / 2) {
            return Offset.INSTANCE.m2461getUnspecifiedF1C5BW0();
        }
        return layoutCoordinates.mo3975localPositionOfR5De75A(innerTextFieldCoordinates, OffsetKt.Offset(coerceIn2, Offset.m2447getYimpl(m2476getCenterF1C5BW0)));
    }
}
