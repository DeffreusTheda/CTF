...
public final class ContentInViewModifier implements BringIntoViewResponder, OnRemeasuredModifier, OnPlacedModifier {
    ...
    public ContentInViewModifier(CoroutineScope scope, Orientation orientation, ScrollableState scrollState, boolean z) {
        ...
        this.viewportSize = IntSize.INSTANCE.m5182getZeroYbymL2g();
    ...
    public Rect calculateRectForParent(Rect localRect) {
        ...
        if (!(!IntSize.m5175equalsimpl0(this.viewportSize, IntSize.INSTANCE.m5182getZeroYbymL2g()))) {
            throw new IllegalStateException("Expected BringIntoViewRequester to not be used before parents are placed.".toString());
        }
        return m281computeDestinationO0kMr_c(localRect, this.viewportSize);
    }

    @Override // androidx.compose.foundation.relocation.BringIntoViewResponder
    public Object bringChildIntoView(Function0<Rect> function0, Continuation<? super Unit> continuation) {
        Rect invoke = function0.invoke();
        if (invoke == null || m283isMaxVisibleO0kMr_c$default(this, invoke, 0L, 1, null)) {
            return Unit.INSTANCE;
        }
    ...
    public void mo285onRemeasuredozmzZPI(long size) {
        Rect focusedChildBounds;
        long j = this.viewportSize;
        this.viewportSize = size;
        if (m279compareToTemP2vQ(size, j) < 0 && (focusedChildBounds = getFocusedChildBounds()) != null) {
            Rect rect = this.focusedChildBoundsFromPreviousRemeasure;
            if (rect == null) {
                rect = focusedChildBounds;
            }
            if (!this.isAnimationRunning && !this.trackingFocusedChild && m282isMaxVisibleO0kMr_c(rect, j) && !m282isMaxVisibleO0kMr_c(focusedChildBounds, size)) {
    ...
    public final float calculateScrollDelta() {
        ...
        long m5187toSizeozmzZPI = IntSizeKt.m5187toSizeozmzZPI(this.viewportSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return relocationDistance(findBringIntoViewRequest.getTop(), findBringIntoViewRequest.getBottom(), Size.m2512getHeightimpl(m5187toSizeozmzZPI));
        }
        if (i == 2) {
            return relocationDistance(findBringIntoViewRequest.getLeft(), findBringIntoViewRequest.getRight(), Size.m2515getWidthimpl(m5187toSizeozmzZPI));
        }
    ...
    private final Rect findBringIntoViewRequest() {
        ...
        if (size > 0) {
            ...
            do {
                ...
                if (invoke != null) {
                    if (m280compareToiLBOSCw(invoke.m2479getSizeNHjbRc(), IntSizeKt.m5187toSizeozmzZPI(this.viewportSize)) > 0) {
                        return rect;
                    }
                    rect = invoke;
                }
    ...
    private final Rect m281computeDestinationO0kMr_c(Rect childBounds, long containerSize) {
        return childBounds.m2483translatek4lQ0M(Offset.m2455unaryMinusF1C5BW0(m284relocationOffsetBMxPBkI(childBounds, containerSize)));
    }

    /* renamed from: isMaxVisible-O0kMr_c$default, reason: not valid java name */
    static /* synthetic */ boolean m283isMaxVisibleO0kMr_c$default(ContentInViewModifier contentInViewModifier, Rect rect, long j, int i, Object obj) {
        if ((i & 1) != 0) {
            j = contentInViewModifier.viewportSize;
        }
        return contentInViewModifier.m282isMaxVisibleO0kMr_c(rect, j);
    }

    /* renamed from: isMaxVisible-O0kMr_c, reason: not valid java name */
    private final boolean m282isMaxVisibleO0kMr_c(Rect rect, long j) {
        return Offset.m2443equalsimpl0(m284relocationOffsetBMxPBkI(rect, j), Offset.INSTANCE.m2462getZeroF1C5BW0());
    }

    /* renamed from: relocationOffset-BMxPBkI, reason: not valid java name */
    private final long m284relocationOffsetBMxPBkI(Rect childBounds, long containerSize) {
        long m5187toSizeozmzZPI = IntSizeKt.m5187toSizeozmzZPI(containerSize);
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return OffsetKt.Offset(0.0f, relocationDistance(childBounds.getTop(), childBounds.getBottom(), Size.m2512getHeightimpl(m5187toSizeozmzZPI)));
        }
        if (i == 2) {
            return OffsetKt.Offset(relocationDistance(childBounds.getLeft(), childBounds.getRight(), Size.m2515getWidthimpl(m5187toSizeozmzZPI)), 0.0f);
        }
        throw new NoWhenBranchMatchedException();
    }
    ...
    private final int m279compareToTemP2vQ(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Intrinsics.compare(IntSize.m5176getHeightimpl(j), IntSize.m5176getHeightimpl(j2));
        }
        if (i == 2) {
            return Intrinsics.compare(IntSize.m5177getWidthimpl(j), IntSize.m5177getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }

    /* renamed from: compareTo-iLBOSCw, reason: not valid java name */
    private final int m280compareToiLBOSCw(long j, long j2) {
        int i = WhenMappings.$EnumSwitchMapping$0[this.orientation.ordinal()];
        if (i == 1) {
            return Float.compare(Size.m2512getHeightimpl(j), Size.m2512getHeightimpl(j2));
        }
        if (i == 2) {
            return Float.compare(Size.m2515getWidthimpl(j), Size.m2515getWidthimpl(j2));
        }
        throw new NoWhenBranchMatchedException();
    }
