...
public final class LazyGridAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        if (this.state.getIsVertical()) {
            m5135getXimpl = IntOffset.m5136getYimpl(lazyGridItemInfo2.getOffset());
        } else {
            m5135getXimpl = IntOffset.m5135getXimpl(lazyGridItemInfo2.getOffset());
        }
        return Integer.valueOf(m5135getXimpl);
    }
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (intValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == intValue) {
                    if (isVertical) {
                        m5177getWidthimpl = IntSize.m5176getHeightimpl(visibleItemsInfo.get(i).getSize());
                    } else {
                        m5177getWidthimpl = IntSize.m5177getWidthimpl(visibleItemsInfo.get(i).getSize());
                    }
                    i4 = Math.max(i4, m5177getWidthimpl);
                    i++;
                }
                i2 += i4;
                i3++;
