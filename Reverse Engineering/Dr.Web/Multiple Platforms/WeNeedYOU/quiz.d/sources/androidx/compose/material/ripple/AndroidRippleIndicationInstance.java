...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f, State<Color> color, State<RippleAlpha> rippleAlpha, RippleContainer rippleContainer) {
        ...
        this.rippleSize = Size.INSTANCE.m2524getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        ...
        this.rippleSize = contentDrawScope.mo3184getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            i = MathKt.roundToInt(RippleAnimationKt.m1007getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo3184getSizeNHjbRc()));
        } else {
            i = contentDrawScope.mo354roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = i;
        long m2721unboximpl = this.color.getValue().m2721unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m1012drawStateLayerH2RKhps(contentDrawScope2, this.radius, m2721unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m1011updateRipplePropertiesbiQXAtU(contentDrawScope.mo3184getSizeNHjbRc(), this.rippleRadius, m2721unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m1010addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m2721unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
