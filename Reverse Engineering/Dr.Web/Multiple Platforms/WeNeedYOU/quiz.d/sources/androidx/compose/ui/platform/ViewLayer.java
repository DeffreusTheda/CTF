...
public final class ViewLayer extends View implements OwnedLayer, GraphicLayerInfo {
    ...
    public ViewLayer(AndroidComposeView ownerView, DrawChildContainer container, Function1<? super Canvas, Unit> drawBlock, Function0<Unit> invalidateParentLayer) {
        ...
        this.mTransformOrigin = TransformOrigin.INSTANCE.m3104getCenterSzJe1aQ();
    ...
    public void mo4235updateLayerPropertiesdDxrwY(float scaleX, float scaleY, float alpha, float translationX, float translationY, float shadowElevation, float rotationX, float rotationY, float rotationZ, float cameraDistance, long transformOrigin, Shape shape, boolean clip, RenderEffect renderEffect, long ambientShadowColor, long spotShadowColor, int compositingStrategy, LayoutDirection layoutDirection, Density density) {
        ...
        setPivotX(TransformOrigin.m3099getPivotFractionXimpl(this.mTransformOrigin) * getWidth());
        setPivotY(TransformOrigin.m3100getPivotFractionYimpl(this.mTransformOrigin) * getHeight());
        ...
        if (Build.VERSION.SDK_INT >= 28) {
            ...
            ViewLayerVerificationHelper28.INSTANCE.setOutlineAmbientShadowColor(viewLayer, ColorKt.m2765toArgb8_81llA(ambientShadowColor));
            ViewLayerVerificationHelper28.INSTANCE.setOutlineSpotShadowColor(viewLayer, ColorKt.m2765toArgb8_81llA(spotShadowColor));
        }
        if (Build.VERSION.SDK_INT >= 31) {
            ViewLayerVerificationHelper31.INSTANCE.setRenderEffect(this, renderEffect);
        }
        if (CompositingStrategy.m2793equalsimpl0(compositingStrategy, CompositingStrategy.INSTANCE.m2799getOffscreenNrFUSI())) {
            setLayerType(2, null);
        } else if (CompositingStrategy.m2793equalsimpl0(compositingStrategy, CompositingStrategy.INSTANCE.m2798getModulateAlphaNrFUSI())) {
            setLayerType(0, null);
            z = false;
        } else {
            setLayerType(0, null);
    ...
    public boolean mo4230isInLayerk4lQ0M(long position) {
        ...
        if (this.clipToBounds) {
            return 0.0f <= m2446getXimpl && m2446getXimpl < ((float) getWidth()) && 0.0f <= m2447getYimpl && m2447getYimpl < ((float) getHeight());
        }
        if (getClipToOutline()) {
    ...
    public void mo4233resizeozmzZPI(long size) {
        ...
        if (m5177getWidthimpl == getWidth() && m5176getHeightimpl == getHeight()) {
            return;
        }
        float f = m5177getWidthimpl;
        setPivotX(TransformOrigin.m3099getPivotFractionXimpl(this.mTransformOrigin) * f);
        float f2 = m5176getHeightimpl;
        setPivotY(TransformOrigin.m3100getPivotFractionYimpl(this.mTransformOrigin) * f2);
        this.outlineResolver.m4327updateuvyYCjk(SizeKt.Size(f, f2));
        updateOutlineResolver();
        layout(getLeft(), getTop(), getLeft() + m5177getWidthimpl, getTop() + m5176getHeightimpl);
        resetClipBounds();
        this.matrixCache.invalidate();
    }
    ...
    public void mo4232movegyyYBs(long position) {
        ...
        if (m5135getXimpl != getLeft()) {
            offsetLeftAndRight(m5135getXimpl - getLeft());
            this.matrixCache.invalidate();
        }
        int m5136getYimpl = IntOffset.m5136getYimpl(position);
        if (m5136getYimpl != getTop()) {
            offsetTopAndBottom(m5136getYimpl - getTop());
            this.matrixCache.invalidate();
        }
    }
    ...
    public long mo4231mapOffset8S9VItk(long point, boolean inverse) {
        if (inverse) {
            float[] m4303calculateInverseMatrixbWbORWo = this.matrixCache.m4303calculateInverseMatrixbWbORWo(this);
        ...
        return androidx.compose.ui.graphics.Matrix.m2950mapMKHz9U(this.matrixCache.m4304calculateMatrixGrdbGEg(this), point);
    }
    ...
    public void mapBounds(MutableRect rect, boolean inverse) {
        ...
        androidx.compose.ui.graphics.Matrix.m2952mapimpl(this.matrixCache.m4304calculateMatrixGrdbGEg(this), rect);
    }
    ...
    public void mo4234transform58bKbWc(float[] matrix) {
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        androidx.compose.ui.graphics.Matrix.m2961timesAssign58bKbWc(matrix, this.matrixCache.m4304calculateMatrixGrdbGEg(this));
    }
