...
public abstract class LazyGridMeasuredLineProvider {
    ...
    public final long m660childConstraintsJhjzzOo$foundation_release(int startSlot, int span) {
        int i;
        if (span == 1) {
        ...
        if (this.isVertical) {
            return Constraints.INSTANCE.m4971fixedWidthOenEA2s(coerceAtLeast);
        }
        return Constraints.INSTANCE.m4970fixedHeightOenEA2s(coerceAtLeast);
    }

    /* renamed from: itemConstraints-OenEA2s, reason: not valid java name */
    public final long m661itemConstraintsOenEA2s(int itemIndex) {
        LazyGridSpanLayoutProvider lazyGridSpanLayoutProvider = this.spanLayoutProvider;
        return m660childConstraintsJhjzzOo$foundation_release(0, lazyGridSpanLayoutProvider.spanOf(itemIndex, lazyGridSpanLayoutProvider.getSlotsPerLine()));
    }
    ...
    public final LazyGridMeasuredLine getAndMeasure(int lineIndex) {
        ...
        for (int i3 = 0; i3 < size; i3++) {
            int m639getCurrentLineSpanimpl = GridItemSpan.m639getCurrentLineSpanimpl(lineConfiguration.getSpans().get(i3).getPackedValue());
            LazyGridMeasuredItem m659getAndMeasure3p2s80s = this.measuredItemProvider.m659getAndMeasure3p2s80s(lineConfiguration.getFirstItemIndex() + i3, i, m660childConstraintsJhjzzOo$foundation_release(i2, m639getCurrentLineSpanimpl));
            i2 += m639getCurrentLineSpanimpl;
            Unit unit = Unit.INSTANCE;
            lazyGridMeasuredItemArr[i3] = m659getAndMeasure3p2s80s;
        }
