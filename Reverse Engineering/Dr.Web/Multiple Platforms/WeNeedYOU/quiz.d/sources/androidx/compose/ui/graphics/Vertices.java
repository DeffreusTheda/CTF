...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m2765toArgb8_81llA(colors.get(i).m2721unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                m2447getYimpl = Offset.m2446getXimpl(packedValue);
            } else {
                m2447getYimpl = Offset.m2447getYimpl(packedValue);
            }
            fArr[i] = m2447getYimpl;
        }
        return fArr;
    }
}
