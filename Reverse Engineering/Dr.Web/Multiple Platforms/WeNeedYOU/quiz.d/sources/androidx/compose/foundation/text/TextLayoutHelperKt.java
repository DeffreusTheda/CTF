...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m879canReuse7_7YC6M(TextLayoutResult canReuse, AnnotatedString text, TextStyle style, List<AnnotatedString.Range<Placeholder>> placeholders, int i, boolean z, int i2, Density density, LayoutDirection layoutDirection, FontFamily.Resolver fontFamilyResolver, long j) {
        ...
        if (canReuse.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() || !Intrinsics.areEqual(layoutInput.getText(), text) || !layoutInput.getStyle().hasSameLayoutAffectingAttributes(style) || !Intrinsics.areEqual(layoutInput.getPlaceholders(), placeholders) || layoutInput.getMaxLines() != i || layoutInput.getSoftWrap() != z || !TextOverflow.m4939equalsimpl0(layoutInput.getOverflow(), i2) || !Intrinsics.areEqual(layoutInput.getDensity(), density) || layoutInput.getLayoutDirection() != layoutDirection || !Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), fontFamilyResolver) || Constraints.m4963getMinWidthimpl(j) != Constraints.m4963getMinWidthimpl(layoutInput.getConstraints())) {
            return false;
        }
        if (z || TextOverflow.m4939equalsimpl0(i2, TextOverflow.INSTANCE.m4947getEllipsisgIe3tQ8())) {
            return Constraints.m4961getMaxWidthimpl(j) == Constraints.m4961getMaxWidthimpl(layoutInput.getConstraints()) && Constraints.m4960getMaxHeightimpl(j) == Constraints.m4960getMaxHeightimpl(layoutInput.getConstraints());
        }
        return true;
    }
}
