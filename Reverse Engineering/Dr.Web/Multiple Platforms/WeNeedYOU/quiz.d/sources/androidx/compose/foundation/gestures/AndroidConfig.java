...
final class AndroidConfig implements ScrollConfig {
    ...
    public long mo278calculateMouseWheelScroll8xgXZGE(Density calculateMouseWheelScroll, PointerEvent event, long j) {
        Intrinsics.checkNotNullParameter(calculateMouseWheelScroll, "$this$calculateMouseWheelScroll");
        Intrinsics.checkNotNullParameter(event, "event");
        List<PointerInputChange> changes = event.getChanges();
        Offset m2435boximpl = Offset.m2435boximpl(Offset.INSTANCE.m2462getZeroF1C5BW0());
        int size = changes.size();
        for (int i = 0; i < size; i++) {
            m2435boximpl = Offset.m2435boximpl(Offset.m2451plusMKHz9U(m2435boximpl.getPackedValue(), changes.get(i).getScrollDelta()));
        }
        return Offset.m2453timestuRUvjQ(m2435boximpl.getPackedValue(), -calculateMouseWheelScroll.mo360toPx0680j_4(Dp.m5017constructorimpl(64)));
    }
}
