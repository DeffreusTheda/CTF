...
public final class AbsoluteCutCornerShape extends CornerBasedShape {
    ...
    public Outline mo764createOutlineLjSzlW0(long size, float topStart, float topEnd, float bottomEnd, float bottomStart, LayoutDirection layoutDirection) {
        ...
        if (topStart + topEnd + bottomStart + bottomEnd == 0.0f) {
            return new Outline.Rectangle(SizeKt.m2536toRectuvyYCjk(size));
        }
        Path Path = AndroidPath_androidKt.Path();
        Path.moveTo(0.0f, topStart);
        Path.lineTo(topStart, 0.0f);
        Path.lineTo(Size.m2515getWidthimpl(size) - topEnd, 0.0f);
        Path.lineTo(Size.m2515getWidthimpl(size), topEnd);
        Path.lineTo(Size.m2515getWidthimpl(size), Size.m2512getHeightimpl(size) - bottomEnd);
        Path.lineTo(Size.m2515getWidthimpl(size) - bottomEnd, Size.m2512getHeightimpl(size));
        Path.lineTo(bottomStart, Size.m2512getHeightimpl(size));
        Path.lineTo(0.0f, Size.m2512getHeightimpl(size) - bottomStart);
        Path.close();
