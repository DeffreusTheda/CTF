...
public final class MultiWidgetSelectionDelegate implements Selectable {
    ...
    private final synchronized int getLastVisibleOffset(TextLayoutResult textLayoutResult) {
        ...
        if (this._previousTextLayoutResult != textLayoutResult) {
            if (textLayoutResult.getDidOverflowHeight() && !textLayoutResult.getMultiParagraph().getDidExceedMaxLines()) {
                lineCount = RangesKt.coerceAtMost(textLayoutResult.getLineForVerticalPosition(IntSize.m5176getHeightimpl(textLayoutResult.getSize())), textLayoutResult.getLineCount() - 1);
                while (textLayoutResult.getLineTop(lineCount) >= IntSize.m5176getHeightimpl(textLayoutResult.getSize())) {
                    lineCount--;
                }
    ...
    public Pair<Selection, Boolean> mo925updateSelectionqCDeeow(long startHandlePosition, long endHandlePosition, Offset previousHandlePosition, boolean isStartHandle, LayoutCoordinates containerLayoutCoordinates, SelectionAdjustment adjustment, Selection previousSelection) {
        ...
        if (layoutCoordinates != null && (invoke = this.layoutResultCallback.invoke()) != null) {
            long mo3975localPositionOfR5De75A = containerLayoutCoordinates.mo3975localPositionOfR5De75A(layoutCoordinates, Offset.INSTANCE.m2462getZeroF1C5BW0());
            return MultiWidgetSelectionDelegateKt.m929getTextSelectionInfoyM0VcXU(invoke, Offset.m2450minusMKHz9U(startHandlePosition, mo3975localPositionOfR5De75A), Offset.m2450minusMKHz9U(endHandlePosition, mo3975localPositionOfR5De75A), previousHandlePosition != null ? Offset.m2435boximpl(Offset.m2450minusMKHz9U(previousHandlePosition.getPackedValue(), mo3975localPositionOfR5De75A)) : null, getSelectableId(), adjustment, previousSelection, isStartHandle);
        }
    ...
    public Selection getSelectAllSelection() {
        Selection m927getAssembledSelectionInfovJH6DeI;
        TextLayoutResult invoke = this.layoutResultCallback.invoke();
        if (invoke == null) {
            return null;
        }
        m927getAssembledSelectionInfovJH6DeI = MultiWidgetSelectionDelegateKt.m927getAssembledSelectionInfovJH6DeI(TextRangeKt.TextRange(0, invoke.getLayoutInput().getText().length()), false, getSelectableId(), invoke);
        return m927getAssembledSelectionInfovJH6DeI;
    }

    @Override // androidx.compose.foundation.text.selection.Selectable
    /* renamed from: getHandlePosition-dBAh8RU, reason: not valid java name */
    public long mo923getHandlePositiondBAh8RU(Selection selection, boolean isStartHandle) {
        TextLayoutResult invoke;
    ...
    public long mo924getRangeOfLineContainingjx7JFs(int offset) {
        TextLayoutResult invoke = this.layoutResultCallback.invoke();
        if (invoke == null) {
