...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString text, long j) {
        ...
        this.selectionStart = TextRange.m4530getMinimpl(j);
        this.selectionEnd = TextRange.m4529getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int m4530getMinimpl = TextRange.m4530getMinimpl(j);
        int m4529getMaximpl = TextRange.m4529getMaximpl(j);
        if (m4530getMinimpl < 0 || m4530getMinimpl > text.length()) {
            throw new IndexOutOfBoundsException("start (" + m4530getMinimpl + ") offset is outside of text region " + text.length());
        }
        if (m4529getMaximpl < 0 || m4529getMaximpl > text.length()) {
            throw new IndexOutOfBoundsException("end (" + m4529getMaximpl + ") offset is outside of text region " + text.length());
        }
        if (m4530getMinimpl <= m4529getMaximpl) {
            return;
        }
        throw new IllegalArgumentException("Do not set reversed range: " + m4530getMinimpl + " > " + m4529getMaximpl);
    }
    ...
    public final TextRange m4675getCompositionMzsxiRA$ui_text_release() {
        if (hasComposition$ui_text_release()) {
            return TextRange.m4520boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* renamed from: getSelection-d9O1mEE$ui_text_release, reason: not valid java name */
    public final long m4676getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int start, int end) {
        ...
        long m4677updateRangeAfterDeletepWDy79M = EditingBufferKt.m4677updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), TextRange);
        setSelectionStart(TextRange.m4530getMinimpl(m4677updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m4529getMaximpl(m4677updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long m4677updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m4677updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), TextRange);
            if (TextRange.m4526getCollapsedimpl(m4677updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
            } else {
                this.compositionStart = TextRange.m4530getMinimpl(m4677updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m4529getMaximpl(m4677updateRangeAfterDeletepWDy79M2);
            }
        }
    }
