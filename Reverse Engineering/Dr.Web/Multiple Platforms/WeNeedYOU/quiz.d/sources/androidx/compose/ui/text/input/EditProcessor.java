...
public final class EditProcessor {
    ...
    private TextFieldValue mBufferState = new TextFieldValue(AnnotatedStringKt.emptyAnnotatedString(), TextRange.INSTANCE.m4537getZerod9O1mEE(), (TextRange) null, (DefaultConstructorMarker) null);
    ...
    public final void reset(TextFieldValue value, TextInputSession textInputSession) {
        ...
        boolean z = true;
        boolean z2 = !Intrinsics.areEqual(value.getComposition(), this.mBuffer.m4675getCompositionMzsxiRA$ui_text_release());
        boolean z3 = false;
        ...
        } else if (TextRange.m4525equalsimpl0(this.mBufferState.getSelection(), value.getSelection())) {
            z = false;
        } else {
            this.mBuffer.setSelection$ui_text_release(TextRange.m4530getMinimpl(value.getSelection()), TextRange.m4529getMaximpl(value.getSelection()));
            z = false;
            z3 = true;
        }
        if (value.getComposition() == null) {
            this.mBuffer.commitComposition$ui_text_release();
        } else if (!TextRange.m4526getCollapsedimpl(value.getComposition().getPackedValue())) {
            this.mBuffer.setComposition$ui_text_release(TextRange.m4530getMinimpl(value.getComposition().getPackedValue()), TextRange.m4529getMaximpl(value.getComposition().getPackedValue()));
        }
        if (z || (!z3 && z2)) {
            this.mBuffer.commitComposition$ui_text_release();
            value = TextFieldValue.m4748copy3r_uNRQ$default(value, (AnnotatedString) null, 0L, (TextRange) null, 3, (Object) null);
        }
    ...
    private final String generateBatchErrorMessage(List<? extends EditCommand> editCommands, final EditCommand failedCommand) {
        ...
        sb.append("Error while applying EditCommand batch to buffer (length=" + this.mBuffer.getLength$ui_text_release() + ", composition=" + this.mBuffer.m4675getCompositionMzsxiRA$ui_text_release() + ", selection=" + ((Object) TextRange.m4535toStringimpl(this.mBuffer.m4676getSelectiond9O1mEE$ui_text_release())) + "):");
    ...
    public final TextFieldValue apply(List<? extends EditCommand> editCommands) {
        ...
        try {
            ...
            TextFieldValue textFieldValue = new TextFieldValue(this.mBuffer.toAnnotatedString$ui_text_release(), this.mBuffer.m4676getSelectiond9O1mEE$ui_text_release(), this.mBuffer.m4675getCompositionMzsxiRA$ui_text_release(), (DefaultConstructorMarker) null);
