...
public final class RippleAnimation {
    ...
    */
    ...
    public final void m1006draw4WTKRHQ(DrawScope draw, long j) {
        Float valueOf;
        Intrinsics.checkNotNullParameter(draw, "$this$draw");
        if (this.startRadius == null) {
            this.startRadius = Float.valueOf(RippleAnimationKt.m1008getRippleStartRadiusuvyYCjk(draw.mo3184getSizeNHjbRc()));
        }
        if (this.targetRadius == null) {
            if (Float.isNaN(this.radius)) {
                valueOf = Float.valueOf(RippleAnimationKt.m1007getRippleEndRadiuscSwnlzA(draw, this.bounded, draw.mo3184getSizeNHjbRc()));
            } else {
                valueOf = Float.valueOf(draw.mo360toPx0680j_4(this.radius));
            }
            this.targetRadius = valueOf;
        }
        if (this.origin == null) {
            this.origin = Offset.m2435boximpl(draw.mo3183getCenterF1C5BW0());
        }
        if (this.targetCenter == null) {
            this.targetCenter = Offset.m2435boximpl(OffsetKt.Offset(Size.m2515getWidthimpl(draw.mo3184getSizeNHjbRc()) / 2.0f, Size.m2512getHeightimpl(draw.mo3184getSizeNHjbRc()) / 2.0f));
        }
        ...
        float m2446getXimpl = Offset.m2446getXimpl(offset.getPackedValue());
        Offset offset2 = this.targetCenter;
        Intrinsics.checkNotNull(offset2);
        float lerp2 = MathHelpersKt.lerp(m2446getXimpl, Offset.m2446getXimpl(offset2.getPackedValue()), this.animatedCenterPercent.getValue().floatValue());
        Offset offset3 = this.origin;
        Intrinsics.checkNotNull(offset3);
        float m2447getYimpl = Offset.m2447getYimpl(offset3.getPackedValue());
        Offset offset4 = this.targetCenter;
        Intrinsics.checkNotNull(offset4);
        long Offset = OffsetKt.Offset(lerp2, MathHelpersKt.lerp(m2447getYimpl, Offset.m2447getYimpl(offset4.getPackedValue()), this.animatedCenterPercent.getValue().floatValue()));
        long m2710copywmQWz5c$default = Color.m2710copywmQWz5c$default(j, Color.m2713getAlphaimpl(j) * floatValue, 0.0f, 0.0f, 0.0f, 14, null);
        if (!this.bounded) {
            DrawScope.CC.m3251drawCircleVaOC9Bg$default(draw, m2710copywmQWz5c$default, lerp, Offset, 0.0f, null, null, 0, MenuKt.InTransitionDuration, null);
            return;
        }
        float m2515getWidthimpl = Size.m2515getWidthimpl(draw.mo3184getSizeNHjbRc());
        float m2512getHeightimpl = Size.m2512getHeightimpl(draw.mo3184getSizeNHjbRc());
        int m2700getIntersectrtfAjoo = ClipOp.INSTANCE.m2700getIntersectrtfAjoo();
        DrawContext drawContext = draw.getDrawContext();
        long mo3190getSizeNHjbRc = drawContext.mo3190getSizeNHjbRc();
        drawContext.getCanvas().save();
        drawContext.getTransform().mo3193clipRectN_I0leg(0.0f, 0.0f, m2515getWidthimpl, m2512getHeightimpl, m2700getIntersectrtfAjoo);
        DrawScope.CC.m3251drawCircleVaOC9Bg$default(draw, m2710copywmQWz5c$default, lerp, Offset, 0.0f, null, null, 0, MenuKt.InTransitionDuration, null);
