...
public final class PlatformMagnifierFactoryApi29Impl implements PlatformMagnifierFactory {
    ...
    public PlatformMagnifierImpl create(MagnifierStyle style, View view, Density density, float initialZoom) {
        ...
        long mo361toSizeXkaWNTQ = density.mo361toSizeXkaWNTQ(style.getSize());
        float mo360toPx0680j_4 = density.mo360toPx0680j_4(style.getCornerRadius());
        float mo360toPx0680j_42 = density.mo360toPx0680j_4(style.getElevation());
        Magnifier.Builder builder = new Magnifier.Builder(view);
        if (mo361toSizeXkaWNTQ != Size.INSTANCE.m2523getUnspecifiedNHjbRc()) {
            builder.setSize(MathKt.roundToInt(Size.m2515getWidthimpl(mo361toSizeXkaWNTQ)), MathKt.roundToInt(Size.m2512getHeightimpl(mo361toSizeXkaWNTQ)));
        }
        if (!Float.isNaN(mo360toPx0680j_4)) {
            builder.setCornerRadius(mo360toPx0680j_4);
        }
        if (!Float.isNaN(mo360toPx0680j_42)) {
            builder.setElevation(mo360toPx0680j_42);
        }
    ...
    public static final class PlatformMagnifierImpl extends PlatformMagnifierFactoryApi28Impl.PlatformMagnifierImpl {
        ...
        public void mo277updateWko1d7g(long sourceCenter, long magnifierCenter, float zoom) {
            if (!Float.isNaN(zoom)) {
                getMagnifier().setZoom(zoom);
            }
            if (OffsetKt.m2465isSpecifiedk4lQ0M(magnifierCenter)) {
                getMagnifier().show(Offset.m2446getXimpl(sourceCenter), Offset.m2447getYimpl(sourceCenter), Offset.m2446getXimpl(magnifierCenter), Offset.m2447getYimpl(magnifierCenter));
            } else {
                getMagnifier().show(Offset.m2446getXimpl(sourceCenter), Offset.m2447getYimpl(sourceCenter));
            }
        }
    }
}
