...
public final class MultiParagraph {
    ...
    private MultiParagraph(MultiParagraphIntrinsics intrinsics, long j, int i, boolean z) {
        boolean z2;
        int m4960getMaxHeightimpl;
        Intrinsics.checkNotNullParameter(intrinsics, "intrinsics");
        this.intrinsics = intrinsics;
        this.maxLines = i;
        if (Constraints.m4963getMinWidthimpl(j) != 0 || Constraints.m4962getMinHeightimpl(j) != 0) {
        ...
        while (i3 < size) {
            ...
            if (Constraints.m4956getHasBoundedHeightimpl(j)) {
                m4960getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m4960getMaxHeightimpl(j) - ParagraphKt.ceilToInt(f), i2);
            } else {
                m4960getMaxHeightimpl = Constraints.m4960getMaxHeightimpl(j);
            }
            Paragraph m4432Paragraph_EkL_Y = ParagraphKt.m4432Paragraph_EkL_Y(intrinsics2, ConstraintsKt.Constraints$default(0, m4961getMaxWidthimpl, 0, m4960getMaxHeightimpl, 5, null), this.maxLines - i4, z);
            float height = f + m4432Paragraph_EkL_Y.getHeight();
            int lineCount = i4 + m4432Paragraph_EkL_Y.getLineCount();
            List<ParagraphIntrinsicInfo> list = infoList$ui_text_release;
            arrayList.add(new ParagraphInfo(m4432Paragraph_EkL_Y, paragraphIntrinsicInfo.getStartIndex(), paragraphIntrinsicInfo.getEndIndex(), i4, lineCount, f, height));
            if (m4432Paragraph_EkL_Y.getDidExceedMaxLines() || (lineCount == this.maxLines && i3 != CollectionsKt.getLastIndex(this.intrinsics.getInfoList$ui_text_release()))) {
                z2 = true;
                i4 = lineCount;
                f = height;
                break;
        ...
        this.width = Constraints.m4961getMaxWidthimpl(j);
    ...
    public static /* synthetic */ void m4418paintRPmYEkk$default(MultiParagraph multiParagraph, Canvas canvas, long j, Shadow shadow, TextDecoration textDecoration, int i, Object obj) {
        if ((i & 2) != 0) {
            j = Color.INSTANCE.m2747getUnspecified0d7_KjU();
        }
        multiParagraph.m4423paintRPmYEkk(canvas, j, (i & 4) != 0 ? null : shadow, (i & 8) != 0 ? null : textDecoration);
    }
    ...
    public final /* synthetic */ void m4423paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            paragraphInfo.getParagraph().mo4395paintRPmYEkk(canvas, color, shadow, decoration);
    ...
    public final void m4422paintLG529CI(Canvas canvas, long color, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo4394paintLG529CI(canvas, color, shadow, decoration, drawStyle, blendMode);
            canvas.translate(0.0f, paragraphInfo.getParagraph().getHeight());
        }
        canvas.restore();
    }
    ...
    public final void m4424painthn5TExg(Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        Intrinsics.checkNotNullParameter(canvas, "canvas");
        Intrinsics.checkNotNullParameter(brush, "brush");
        AndroidMultiParagraphDrawKt.m4760drawMultiParagraph7AXcY_I(this, canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
    }
    ...
    public final Path getPathForRange(int start, int end) {
        ...
        for (int findParagraphByIndex = MultiParagraphKt.findParagraphByIndex(this.paragraphInfoList, start); findParagraphByIndex < size; findParagraphByIndex++) {
            ...
            if (paragraphInfo.getStartIndex() != paragraphInfo.getEndIndex()) {
                Path.CC.m2984addPathUv8p0NA$default(Path, paragraphInfo.toGlobal(paragraphInfo.getParagraph().getPathForRange(paragraphInfo.toLocalIndex(start), paragraphInfo.toLocalIndex(end))), 0L, 2, null);
            }
        }
        return Path;
    }
    ...
    public final int m4420getOffsetForPositionk4lQ0M(long position) {
        int lastIndex;
        if (Offset.m2447getYimpl(position) <= 0.0f) {
            lastIndex = 0;
        } else {
            lastIndex = Offset.m2447getYimpl(position) >= this.height ? CollectionsKt.getLastIndex(this.paragraphInfoList) : MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.m2447getYimpl(position));
        }
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(lastIndex);
        if (paragraphInfo.getLength() == 0) {
            return Math.max(0, paragraphInfo.getStartIndex() - 1);
        }
        return paragraphInfo.toGlobalIndex(paragraphInfo.getParagraph().mo4392getOffsetForPositionk4lQ0M(paragraphInfo.m4429toLocalMKHz9U(position)));
    }
    ...
    public final long m4421getWordBoundaryjx7JFs(int offset) {
        int findParagraphByIndex;
        requireIndexInRangeInclusiveEnd(offset);
        if (offset == getAnnotatedString().length()) {
            findParagraphByIndex = CollectionsKt.getLastIndex(this.paragraphInfoList);
        ...
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(findParagraphByIndex);
        return paragraphInfo.m4428toGlobalGEjPoXI(paragraphInfo.getParagraph().mo4393getWordBoundaryjx7JFs(paragraphInfo.toLocalIndex(offset)));
    }
