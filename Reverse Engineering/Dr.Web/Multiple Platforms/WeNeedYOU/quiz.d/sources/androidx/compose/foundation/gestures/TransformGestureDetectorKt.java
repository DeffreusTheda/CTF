...
public final class TransformGestureDetectorKt {
    ...
    public static final float calculateRotation(PointerEvent pointerEvent) {
        ...
        for (int i4 = 0; i4 < size2; i4++) {
            ...
            if (pointerInputChange2.getPressed() && pointerInputChange2.getPreviousPressed()) {
                ...
                long m2450minusMKHz9U = Offset.m2450minusMKHz9U(pointerInputChange2.getPreviousPosition(), calculateCentroid2);
                long m2450minusMKHz9U2 = Offset.m2450minusMKHz9U(position, calculateCentroid);
                float m387anglek4lQ0M = m387anglek4lQ0M(m2450minusMKHz9U2) - m387anglek4lQ0M(m2450minusMKHz9U);
                float m2444getDistanceimpl = Offset.m2444getDistanceimpl(Offset.m2451plusMKHz9U(m2450minusMKHz9U2, m2450minusMKHz9U)) / 2.0f;
                if (m387anglek4lQ0M > 180.0f) {
                    m387anglek4lQ0M -= 360.0f;
                } else if (m387anglek4lQ0M < -180.0f) {
                    m387anglek4lQ0M += 360.0f;
                }
                f2 += m387anglek4lQ0M * m2444getDistanceimpl;
                f += m2444getDistanceimpl;
            }
        }
        if (f == 0.0f) {
            return 0.0f;
    ...
    private static final float m387anglek4lQ0M(long j) {
        if (Offset.m2446getXimpl(j) == 0.0f && Offset.m2447getYimpl(j) == 0.0f) {
            return 0.0f;
        }
        return ((-((float) Math.atan2(Offset.m2446getXimpl(j), Offset.m2447getYimpl(j)))) * 180.0f) / 3.1415927f;
    }
    ...
    public static final long calculatePan(PointerEvent pointerEvent) {
        Intrinsics.checkNotNullParameter(pointerEvent, "<this>");
        long calculateCentroid = calculateCentroid(pointerEvent, true);
        if (Offset.m2443equalsimpl0(calculateCentroid, Offset.INSTANCE.m2461getUnspecifiedF1C5BW0())) {
            return Offset.INSTANCE.m2462getZeroF1C5BW0();
        }
        return Offset.m2450minusMKHz9U(calculateCentroid, calculateCentroid(pointerEvent, false));
    }
    ...
    public static final float calculateCentroidSize(PointerEvent pointerEvent, boolean z) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            ...
            if (pointerInputChange.getPressed() && pointerInputChange.getPreviousPressed()) {
                f += Offset.m2444getDistanceimpl(Offset.m2450minusMKHz9U(z ? pointerInputChange.getPosition() : pointerInputChange.getPreviousPosition(), calculateCentroid));
                i++;
            }
        }
        return f / i;
    ...
    public static final long calculateCentroid(PointerEvent pointerEvent, boolean z) {
        Intrinsics.checkNotNullParameter(pointerEvent, "<this>");
        long m2462getZeroF1C5BW0 = Offset.INSTANCE.m2462getZeroF1C5BW0();
        List<PointerInputChange> changes = pointerEvent.getChanges();
        int size = changes.size();
        int i = 0;
        for (int i2 = 0; i2 < size; i2++) {
            PointerInputChange pointerInputChange = changes.get(i2);
            if (pointerInputChange.getPressed() && pointerInputChange.getPreviousPressed()) {
                m2462getZeroF1C5BW0 = Offset.m2451plusMKHz9U(m2462getZeroF1C5BW0, z ? pointerInputChange.getPosition() : pointerInputChange.getPreviousPosition());
                i++;
            }
        }
        if (i == 0) {
        ...
        return Offset.m2441divtuRUvjQ(m2462getZeroF1C5BW0, i);
    }
}
