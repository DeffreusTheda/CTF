...
public interface SelectionAdjustment {
    ...
    long mo933adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange);
    ...
    public static final class Companion {
        ...
        private static final SelectionAdjustment None = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$None$1
            ...
            public long mo933adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
        ...
        private static final SelectionAdjustment Character = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Character$1
            ...
            public long mo933adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
                if (TextRange.m4526getCollapsedimpl(newRawSelectionRange)) {
                    return SelectionAdjustmentKt.ensureAtLeastOneChar(textLayoutResult.getLayoutInput().getText().getText(), TextRange.m4532getStartimpl(newRawSelectionRange), StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText()), isStartHandle, previousSelectionRange != null ? TextRange.m4531getReversedimpl(previousSelectionRange.getPackedValue()) : false);
                }
                return newRawSelectionRange;
            }
        };
        private static final SelectionAdjustment Word = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Word$1
            ...
            /* renamed from: adjust-ZXO7KMw */
            public long mo933adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                long m935adjustByBoundaryDvylE;
                Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
                m935adjustByBoundaryDvylE = SelectionAdjustment.Companion.$$INSTANCE.m935adjustByBoundaryDvylE(textLayoutResult, newRawSelectionRange, new SelectionAdjustment$Companion$Word$1$adjust$1(textLayoutResult));
                return m935adjustByBoundaryDvylE;
            }
        };
        private static final SelectionAdjustment Paragraph = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$Paragraph$1
            ...
            public long mo933adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                long m935adjustByBoundaryDvylE;
                Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
                m935adjustByBoundaryDvylE = SelectionAdjustment.Companion.$$INSTANCE.m935adjustByBoundaryDvylE(textLayoutResult, newRawSelectionRange, new SelectionAdjustment$Companion$Paragraph$1$adjust$boundaryFun$1(textLayoutResult.getLayoutInput().getText()));
                return m935adjustByBoundaryDvylE;
            }
        };
        private static final SelectionAdjustment CharacterWithWordAccelerate = new SelectionAdjustment() { // from class: androidx.compose.foundation.text.selection.SelectionAdjustment$Companion$CharacterWithWordAccelerate$1
            ...
            public long mo933adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
                ...
                if (previousSelectionRange == null) {
                    return SelectionAdjustment.Companion.$$INSTANCE.getWord().mo933adjustZXO7KMw(textLayoutResult, newRawSelectionRange, previousHandleOffset, isStartHandle, previousSelectionRange);
                }
                if (TextRange.m4526getCollapsedimpl(newRawSelectionRange)) {
                    return SelectionAdjustmentKt.ensureAtLeastOneChar(textLayoutResult.getLayoutInput().getText().getText(), TextRange.m4532getStartimpl(newRawSelectionRange), StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText()), isStartHandle, TextRange.m4531getReversedimpl(previousSelectionRange.getPackedValue()));
                }
                if (isStartHandle) {
                    i = updateSelectionBoundary(textLayoutResult, TextRange.m4532getStartimpl(newRawSelectionRange), previousHandleOffset, TextRange.m4532getStartimpl(previousSelectionRange.getPackedValue()), TextRange.m4527getEndimpl(newRawSelectionRange), true, TextRange.m4531getReversedimpl(newRawSelectionRange));
                    updateSelectionBoundary = TextRange.m4527getEndimpl(newRawSelectionRange);
                } else {
                    int m4532getStartimpl = TextRange.m4532getStartimpl(newRawSelectionRange);
                    updateSelectionBoundary = updateSelectionBoundary(textLayoutResult, TextRange.m4527getEndimpl(newRawSelectionRange), previousHandleOffset, TextRange.m4527getEndimpl(previousSelectionRange.getPackedValue()), TextRange.m4532getStartimpl(newRawSelectionRange), false, TextRange.m4531getReversedimpl(newRawSelectionRange));
                    i = m4532getStartimpl;
                }
            ...
            private final int snapToWordBoundary(TextLayoutResult textLayoutResult, int newRawOffset, int currentLine, int otherBoundaryOffset, boolean isStart, boolean isReversed) {
                int lineStart;
                int lineEnd$default;
                long m4506getWordBoundaryjx7JFs = textLayoutResult.m4506getWordBoundaryjx7JFs(newRawOffset);
                if (textLayoutResult.getLineForOffset(TextRange.m4532getStartimpl(m4506getWordBoundaryjx7JFs)) == currentLine) {
                    lineStart = TextRange.m4532getStartimpl(m4506getWordBoundaryjx7JFs);
                } else {
                    lineStart = textLayoutResult.getLineStart(currentLine);
                }
                if (textLayoutResult.getLineForOffset(TextRange.m4527getEndimpl(m4506getWordBoundaryjx7JFs)) == currentLine) {
                    lineEnd$default = TextRange.m4527getEndimpl(m4506getWordBoundaryjx7JFs);
                } else {
            ...
            private final boolean isAtWordBoundary(TextLayoutResult textLayoutResult, int i) {
                long m4506getWordBoundaryjx7JFs = textLayoutResult.m4506getWordBoundaryjx7JFs(i);
                return i == TextRange.m4532getStartimpl(m4506getWordBoundaryjx7JFs) || i == TextRange.m4527getEndimpl(m4506getWordBoundaryjx7JFs);
            }
        };
        ...
        public final long m935adjustByBoundaryDvylE(TextLayoutResult textLayoutResult, long newRawSelection, Function1<? super Integer, TextRange> boundaryFun) {
            if (textLayoutResult.getLayoutInput().getText().length() == 0) {
                return TextRange.INSTANCE.m4537getZerod9O1mEE();
            }
            int lastIndex = StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText());
            long packedValue = boundaryFun.invoke(Integer.valueOf(RangesKt.coerceIn(TextRange.m4532getStartimpl(newRawSelection), 0, lastIndex))).getPackedValue();
            long packedValue2 = boundaryFun.invoke(Integer.valueOf(RangesKt.coerceIn(TextRange.m4527getEndimpl(newRawSelection), 0, lastIndex))).getPackedValue();
            return TextRangeKt.TextRange(TextRange.m4531getReversedimpl(newRawSelection) ? TextRange.m4527getEndimpl(packedValue) : TextRange.m4532getStartimpl(packedValue), TextRange.m4531getReversedimpl(newRawSelection) ? TextRange.m4532getStartimpl(packedValue2) : TextRange.m4527getEndimpl(packedValue2));
        }
