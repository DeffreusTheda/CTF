...
public final class ShaderBrushSpan extends CharacterStyle implements UpdateAppearance {
    ...
    public ShaderBrushSpan(ShaderBrush shaderBrush, float f) {
        ...
        this.alpha = f;
        this.size = Size.INSTANCE.m2523getUnspecifiedNHjbRc();
    }
    ...
    public final void m4791setSizeuvyYCjk(long j) {
        this.size = j;
    }

    @Override // android.text.style.CharacterStyle
    public void updateDrawState(TextPaint textPaint) {
        Shader mo2680createShaderuvyYCjk;
        Intrinsics.checkNotNullParameter(textPaint, "textPaint");
        AndroidTextPaint_androidKt.setAlpha(textPaint, this.alpha);
        if (this.size == Size.INSTANCE.m2523getUnspecifiedNHjbRc()) {
            return;
        }
        Pair<Size, ? extends Shader> pair = this.cachedShader;
        if (pair == null || !Size.m2511equalsimpl0(pair.getFirst().getPackedValue(), this.size)) {
            mo2680createShaderuvyYCjk = this.shaderBrush.mo2680createShaderuvyYCjk(this.size);
        } else {
            mo2680createShaderuvyYCjk = pair.getSecond();
        }
        textPaint.setShader(mo2680createShaderuvyYCjk);
        this.cachedShader = TuplesKt.to(Size.m2503boximpl(this.size), mo2680createShaderuvyYCjk);
    }
}
