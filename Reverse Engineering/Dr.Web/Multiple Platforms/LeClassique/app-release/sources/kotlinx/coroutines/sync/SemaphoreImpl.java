...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
            ...
            while (true) {
                if (segment2.getId() < j || segment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        m1679constructorimpl = SegmentOrClosed.m1679constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    m1679constructorimpl = SegmentOrClosed.m1679constructorimpl(segment2);
                    break;
                }
            }
            if (!SegmentOrClosed.m1684isClosedimpl(m1679constructorimpl)) {
                Segment m1682getSegmentimpl = SegmentOrClosed.m1682getSegmentimpl(m1679constructorimpl);
                while (true) {
                    Segment segment4 = (Segment) this.tail;
                    if (segment4.getId() >= m1682getSegmentimpl.getId()) {
                        break loop0;
                    }
                    if (!m1682getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        break;
                    }
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment4, m1682getSegmentimpl)) {
                        if (segment4.decPointers$kotlinx_coroutines_core()) {
                            segment4.remove();
                        }
                    } else if (m1682getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m1682getSegmentimpl.remove();
                    }
                }
            } else {
                break;
        ...
        SemaphoreSegment semaphoreSegment = (SemaphoreSegment) SegmentOrClosed.m1682getSegmentimpl(m1679constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        loop0: while (true) {
            ...
            while (true) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment4, m1682getSegmentimpl)) {
                    if (segment4.decPointers$kotlinx_coroutines_core()) {
                        segment4.remove();
                    }
                } else if (m1682getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
