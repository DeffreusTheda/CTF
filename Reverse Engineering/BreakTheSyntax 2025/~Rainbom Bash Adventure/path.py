...
def optimize_large_tsp(distance_matrix, time_limit=60, chunk_size=None):
    ...
    # If the problem is small enough, just use Held-Karp
    if n <= 15:
...
def run_tsp_solver(matrix_file=None, random=False, size=20, seed=42, time_limit=120, custom_matrix=None):
    ...
    if custom_matrix is not None:
        ...
        dist_matrix = np.array(custom_matrix)
    elif random:
        ...
        try:
            dist_matrix = np.loadtxt(matrix_file, delimiter=',')
        except Exception as e:
...
if __name__ == "__main__":
    ...
    opt_distance, opt_path = run_tsp_solver(custom_matrix=my_matrix, time_limit=300)
